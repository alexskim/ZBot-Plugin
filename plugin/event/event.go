		...
		Handle(func(ctx *zero.Ctx) {
			...
			if ok {
				...
				ctx.SendPrivateForwardMessage(su,
					message.Message{
						message.CustomNode(username, userid,
							"在"+now+"收到来自"+
								"\n用户:["+username+"]("+strconv.FormatInt(userid, 10)+")的群聊邀请"+
								"\n群聊:["+groupname+"]("+strconv.FormatInt(groupid, 10)+")",
						),
						message.CustomNode(username, userid, "同意邀请"+es),
						message.CustomNode(username, userid, "拒绝邀请"+es),
					})
			}
		})
					...
					message.Message{
						message.CustomNode(username, userid,
							"在"+now+"收到来自"+
								"\n用户:["+username+"]("+strconv.FormatInt(userid, 10)+")"+
								"\n的好友请求:"+comment),
						message.CustomNode(username, userid, "同意申请"+es),
						message.CustomNode(username, userid, "拒绝申请"+es),
					})
			}
		})
